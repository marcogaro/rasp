...
class Operations(pyfuse3.Operations):
    ...
    def _add_path(self, inode, path):
        ...
        for nome in listgpio:
            if path == '/sys/class/gpio/' + nome or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/gpio') or path == '/sys/devices/platform/soc/3f200000.gpio/gpio/gpiochip0' or path == '/sys/devices/platform/soc/3f200000.gpio/gpiochip0' or path == '/sys/devices/platform/soc/3f200000.gpio/gpiochip0/gpio' or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/gpiochip0/gpio/' + nome) or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/driver') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/driver_override') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/modalias') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/of_node') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/power') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/subsystem') or path.startswith(
                    '/sys/devices/platform/soc/3f200000.gpio/uevent'):
                print("primo if")
                print("nome: ", nome)
                print("path: ", path)
                print('/sys/class/gpio/' + nome)
                if '/sys/class/gpio/' + nome == '/sys/class/gpio/export':
                    print("1caso")
                    export.clear()
                    export.insert(0, 1)
                    unexport.clear()
                    unexport.insert(0, 0)
                elif '/sys/class/gpio/' + nome == '/sys/class/gpio/unexport':
                    print("2caso")
                    export.clear()
                    export.insert(0, 0)
                    unexport.clear()
    ...
    async def write(self, fd, offset, buf):
        ...
        if export[0] == 1 and unexport[0] == 0:
            ...
            if ff in listgpio:
                ...
                print("errore")
                string = "0"
                fa = bytes(string, 'utf-8')
                print("newbuf: ", fa)
                os.lseek(fd, offset, os.SEEK_SET)
                return 1
        elif export[0] == 0 and unexport[0] == 1:
            print("1unexport")
            print('/sys/class/gpio/gpio11/')
            print(os.path.exists('/sys/class/gpio/gpio11/'))
            if ff in listgpio and os.path.exists('/sys/class/gpio/gpio11/') :
                os.lseek(fd, offset, os.SEEK_SET)
                return os.write(fd, buf)
            else:
                print("errore")
